...
| **XP** | 655 |
| **Phase** | 1 |
| **Today** | **Day 25 ✅ DONE** (Sat Jul 11 IST) — shipped his **first browser-served chatbot**: wrapped the Day-24 `aiBot` in a Gradio `gr.ChatInterface` on localhost (`http://127.0.0.1:7860`) — a real web chat UI (input, thread, retry/clear) from one `.launch()`. Gave it **per-session memory the correct way**: a `gr.State(None)` threaded through `additional_inputs` **and** `additional_outputs`, so `previous_interaction_id` survives turn-to-turn scoped to each browser session (never a module global). New tool installed: **Gradio 6.20** (`uv add gradio`; resolved clean on the Python-3.14 pin, 39 pkgs — no wheel wall). **Concept cracked:** a Gradio component in an inputs list passes its *value, not itself* — `state` (the box) is unwrapped to its contents (the id) before it reaches the `prev_id` param, and the returned id is written back into the box via `additional_outputs`. **Day-24 lesson reinforced, framework-enforced:** first cut reassigned a dead local (`prev_id = ...`) and returned only the reply → the id vanished → fixed by *returning* `(reply, new_id)` and letting the caller (Gradio's session store) hold it — same "report-back-via-return, don't-stash-a-local/global" move. Verified live: "my name is Vraj" → "what's my name?" → **it remembered.** New file `phase1/week4/day25.py`. **Next — Day 27 (Mon Jul 13): deploy this exact app to a Hugging Face Space = public URL → 🥉 First Deploy** (Day 26 = Sun Jul 12 = mandatory rest). Deploy needs `app.py` **self-contained** (inline `aiBot`, drop the cross-file import) + `requirements.txt` (gradio, google-genai) + README `sdk: gradio` front-matter + API key as a Space **Secret** (`GEMINI_API_KEY`, which `genai.Client()` auto-reads). |
| **This week's sub-goal** | **Week 4 (Days 22–28) — build-first pivot:** re-entry 22 ✅ → 🥉 First Token (23) ✅ → interactive chatbot with memory + error shield (24) ✅ → **Gradio browser chat UI with per-session memory (25) ✅**. 🔜 **Deploy arc:** Day 26 = Sunday REST → **Day 27 (Mon): push to a Hugging Face Space = public URL 🥉 First Deploy** → polish/streaming + share the link (28). Old bottom-up bias/two-parameter/🥉 First Model thread still *parked* (backfilled just-in-time), not abandoned. |
| **Streak** | 3 (Days 23–25 ✅). Three build-first days in a row after the ~1-month gap reset — momentum is real; deploy is the payoff. |
...
| **Next trophy in sight** | 🥉 **First Deploy** — this chatbot live at a public URL (Gradio on a Hugging Face Space), **Day 27 (Mon Jul 13)**. The browser app is built and memory-verified today; Monday is just the push + Space Secret + `requirements.txt`. Day 25 built the thing; Day 27 makes it public. ≤3-day cadence holds. |
