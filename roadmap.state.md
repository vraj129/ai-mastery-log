...
| **XP** | 620 |
| **Phase** | 1 |
| **Today** | **Day 24 ✅ DONE** (Fri Jul 10 IST) — first **interactive multi-turn chatbot** with memory + graceful error handling, in `phase1/week4/day24.py`. Turned Day-23's one-shot call into a live CLI chat loop that **remembers across turns** (Interactions API + `previous_interaction_id` chaining — his OWN approach, not the `chats.create()` hint given). Fought through **three real production lessons via live errors**: (1) **per-project quota** — new API keys don't reset the free-tier 429 (quota is per Google-Cloud-project, not per key; RPD resets midnight US-Pacific); (2) **model selection by cost/limit** — premium `gemini-3.5-flash` free tier is stingy → switched to `gemini-3.1-flash-lite` (15 RPM / 1000 RPD, fresh bucket); (3) **model-ID churn** — `gemini-2.5-flash-lite` 404'd ("no longer available to new users") → check the live models page. Then **two self-inflicted-and-fixed bugs that cemented fundamentals**: *local vars don't leak to the caller* (moved `prev_id` global→param → memory silently broke → fixed by **returning** the new id and catching it in the loop), and *all return paths must share one shape* (success returned a 2-tuple, `except` returned 1 → `ValueError: too many values to unpack` crash on a 429 → fixed with `(msg, prev_id)`, which also preserves memory through the hiccup). Self-cleaned the sentinel to `None` throughout, unprompted. **Next (Day 25): wrap this exact bot in a Gradio `gr.ChatInterface` and chat with it in a browser on localhost** — the bridge to 🥉 First Deploy (HF Spaces, ~Day 26). New file `phase1/week4/day25.py`; resource-first = gradio.app "creating-a-chatbot-fast" guide. |
| **This week's sub-goal** | **Week 4 (Days 22–28) — build-first pivot:** re-entry 22 ✅ → 🥉 First Token (23) ✅ → interactive chatbot with memory + error shield (24) ✅. 🔜 **Deploy arc:** Gradio chat UI on localhost (25) → push to a Hugging Face Space = public URL 🥉 **First Deploy** (26) → polish/streaming + share the link (27). The old bottom-up bias/two-parameter/🥉 First Model thread is *parked* (backfilled just-in-time when a build needs regression math), not abandoned. |
| **Streak** | 2 (Days 23–24 ✅). Fresh streak after the ~1-month gap reset (knowledge + Git history fully intact). Two build-first days back to back — momentum is real. |
...
| **Next trophy in sight** | 🥉 **First Deploy** — this chatbot live at a public URL (Gradio on a Hugging Face Space), ~Day 26. Day 23 popped 🥉 First Token; Day 24 = a solid build+debug day (chatbot with memory + error handling; no new trophy, 3 production lessons banked). ≤3-day cadence holds: First Deploy is the pop at ~26. |
