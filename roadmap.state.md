...
| **XP** | 735 |
| **Phase** | 1 |
| **Today** | **Day 28 ✅ DONE — streaming chatbot (live token typing)** (Tue Jul 14 IST) — made the local twin of the deployed bot **type word-by-word**, like a real AI product. Turned Day-25's one-shot `random_response` into a **Python generator**: `client.interactions.create(..., stream=True)` returns an **event stream**; he grabs the new interaction id from the first `interaction.created` event, accumulates `step.delta` text, and **`yield`s the growing string + id each step** so Gradio repaints the bubble live. Kept per-session memory via `gr.State` — **stream and memory are independent** (one `stream=True` flag, no rewrite of the `previous_interaction_id` logic). **Verified live locally:** it types as it thinks AND still remembers his name across turns. **The real win = the debugging loop:** wrote it, hit a **blank-bubble** bug (wrong event-type string `"interaction"` → `"step.delta"`, so the accumulate branch never fired), then a **sleeper** (`return` inside a generator silently swallows the value → must `yield`) — diagnosed both from evidence and iterated twice to green. New file `phase1/week4/day28.py`; gitignored the `.gradio/` cache; commit `b7cb99c` on `main`. **Context this session:** briefly explored a personal **RAG-over-a-Gujarati-Granth** idea → parked as a future **Phase-3 flagship** (verified feasible & $0 — Gemini reads Gujarati scans, cross-lingual embeddings handle any-language questions; gated on **community permission** to digitize the edition). Also had an honest **"am I actually growing or just prompting ideas?"** gut-check — answer landed: the write→break→read-the-evidence→fix loop he ran today **is** the engineering muscle. **Next — Day 29:** loop back toward 🥉 **First Model** (NumPy logistic regression) OR pop 🥉 **PR Author** by shipping the streaming redeploy via a branch + PR-to-self. |
| **Today (archived Day 27)** | **🥉 First Deploy** (Mon Jul 13 IST) — **shipped his first LIVE public app.** The Day-25 Gradio chatbot now runs on a **Render free Web Service** at a public URL — **https://gemini-chatbot-tw9q.onrender.com/** — reachable by anyone, and it stays up whether his laptop is open or closed. **Verified live in the cloud:** "my name is Vraj" → "what's my name?" → **"Vraj"** (proves both the API key works on the host AND per-session `gr.State` memory survives on a real server). **Platform pivot mid-flight:** Hugging Face **paywalled Gradio/Docker Spaces ~Jul 8–9** (now PRO $9/mo; only Static is free, and Static can't run Python) — he *surfaced the paywall himself* instead of clicking through, then adapted **HF Spaces → Render** (his roadmap's own documented $0 fallback) instead of quitting. **What he built:** self-contained `phase1/week4/deploy/app.py` (inlined `aiBot`, dropped the cross-file import = the #1 deploy blocker) binding `server_name="0.0.0.0"` + `server_port=int(os.environ.get("PORT", 7860))` (host bind + port injection), plus a pinned `requirements.txt` (gradio==6.20.0, google-genai==2.10.0). Grabbed a temporary `*.gradio.live` **share link** first (Beat A) to feel the win + learn tunnels-vs-hosts. Commit `981cdb6` pushed to GitHub; Render root dir = `phase1/week4/deploy`, build `pip install -r requirements.txt`, start `python app.py`, `GEMINI_API_KEY` as a Render secret. **Next — Day 28: consolidate/share** (bookmark the URL, optional polish: streaming or `auth`; ship the learn-in-public post on Saturday), then loop back to Phase-1 core (🥉 First Model). |
| **This week's sub-goal** | **Week 4 (Days 22–28) — build-first pivot:** re-entry 22 ✅ → 🥉 First Token (23) ✅ → chatbot w/ memory + error shield (24) ✅ → Gradio browser UI w/ per-session memory (25) ✅ → Sunday rest (26) → 🥉 First Deploy on Render (27) ✅ → **streaming / live-typing chatbot (28) ✅**. **Deploy + polish arc COMPLETE.** 🔜 loop back to Phase-1 core (🥉 First Model) and/or pop 🥉 PR Author (streaming redeploy via a PR-to-self). Old bottom-up threads (two-parameter / 🥉 First Model) still *parked* (backfilled just-in-time), not abandoned. |
| **Streak** | 5 (Days 23–25, 27, 28 ✅; Sunday 26 = neutral rest). Five build-first days: token → memory → browser UI → public deploy → live streaming, plus a real two-iteration debugging loop today. Momentum strong, proven under adversity. |
...
| **Next trophy in sight** | 🥉 **First Deploy — EARNED (Day 27)**. Day 28 = a no-trophy feature/skill day (streaming) — fine, within the ≤3-day cadence. Next reachable pops: 🥉 **PR Author** (first squash-merged PR-to-self — a natural reliability-spine step; could BE the streaming redeploy) or 🥉 **First Model** (NumPy logistic regression beats a baseline — resumes the parked Phase-1 core). |
...
Deploy ($0, verified Jul 2026): GitHub → Render free Web Service (bind 0.0.0.0 + $PORT; ~15-min idle spin-down). ⚠️ Hugging Face PAYWALLED Gradio/Docker Spaces ~Jul 8 2026 (PRO $9/mo; only Static free) — Render is the default $0 host now
